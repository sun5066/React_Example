...
/**
 * myarray 요소를 foreach 문으로 반복하는 것과 같이 순회하면서
 * 각 요소를 arr 변수에 넘기고 내부의 코드를 실행해서
 * return 하면 새로운 배열을 만드는 코드
 * 기존의 배열을 변경하기 않고, 어떤 연산을 수행한 후
 * 똑같은 개수의 배열을 만들어 준다.
 *
 * JS의 순수함수
 * 함수의 전달된 매개변수를 직접 변경하지 않고
 * 연산을 수행한 후 다른 변수에 return 하여 값을 담는 구조
 */
const mapArr = myarray.map((arr) => {
    // arr++; map 함수내부에서 myarray 의 요소를 변경하는 코드는
    // 가급적 지양하라
    return arr + 1;
});
 ...
 * 새로운 배열에 담아주는 함수
 */
const filterArr = myarray.filter((arr, index) => {
